    ...
    return (
      <div className="min-h-screen text-white p-6">
        <div className="max-w-4xl mx-auto">
          ...
          {/* Transaction History for this land */}
          <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-6 border border-white/20">
            <h2 className="text-2xl font-bold mb-6">Historique complet du terrain</h2>
                ...
                <div key={index} className="flex gap-4 mb-6 relative">
                  ...
                  <div className="flex-1 bg-slate-900/60 rounded-2xl p-4 border border-white/10">
                    <div className="flex items-center gap-3 mb-2 flex-wrap">
                      ...
                      <span className="text-white/50 text-sm">Bloc #{item.block}</span>
                    </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-8">
          <form onSubmit={mintLand} className="bg-white/10 backdrop-blur-xl rounded-3xl p-6 border border-white/20">
            ...
            <div className="space-y-3">
              <input
                className="w-full p-3 rounded-2xl text-black"
              ...
              <input
                ...
                value={mintForm.areaSqMeters}
                onChange={(e) => setMintForm({ ...mintForm, areaSqMeters: e.target.value })}
              />
          ...
          <form onSubmit={listForSale} className="bg-white/10 backdrop-blur-xl rounded-3xl p-6 border border-white/20">
            ...
            <div className="space-y-3">
              <input
                ...
                value={saleForm.tokenId}
                onChange={(e) => setSaleForm({ ...saleForm, tokenId: e.target.value })}
              />
              <input
                className="w-full p-3 rounded-2xl text-black"
        ...
        <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-6 border border-white/20 mb-8">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              onClick={() => { loadLands(); loadHistory(); }}
              className="px-4 py-2 rounded-2xl bg-sky-500 hover:bg-sky-600 font-semibold"
            >
              Actualiser
            </button>
          </div>
        ...
        <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-6 border border-white/20">
          <h2 className="text-2xl font-bold mb-4">Historique global des transactions</h2>
          <div className="space-y-3">
            {history.length === 0 && (
              <p className="text-white/60 text-center py-4">Aucune transaction trouvée</p>
            )}
            {history.map((item, index) => (
              <div key={index} className="bg-slate-900/60 rounded-2xl p-4 border border-white/10 flex flex-col md:flex-row md:items-center gap-3">
                ...
                <button
                  onClick={() => {
                    const land = lands.find(l => l.id === item.tokenId);
                    if (land) openLandDetail(land);
                  }}
